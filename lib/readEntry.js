...
// Imports
const {readAndAssertNoEof, emitErrorAndAutoClose} = require('./internals');
const {
...
function emittedEntry(entry, cb) {
	...
	if (zipFile._readingAt) return readAtEntry(zipFile, entry);
...
function emittedError(err, cb) {
	...
	if (zipFile._readingAt) return readAtError(zipFile, err);
...
function readEntryDone(zipFile, entry, cb) {
	...
	if (!zipFile.supportMacArchiveUtility || !zipFile.isMacArchive) {
		...
		return emitEntry(zipFile, entry, cb);
	}
	...
	if (entry.relativeOffsetOfLocalHeader != offset % FOUR_GIB) return emitError(zipFile);
	...
	if (entry.compressionMethod == 0) {
		...
		return found(zipFile, entry, cb);
	}

	entry.uncompressedSizeCertain = false;

	if (offset + LF_MIN_LENGTH * (zipFile.entryCount - zipFile.entriesRead) + FOUR_GIB > zipFile.centralDirectoryOffset) return found(zipFile, entry, cb);
...
function searchFileEnd(zipFile, entry, size, offset, cb) {
	...
	readAndAssertNoEof(zipFile.reader, buffer, 0, bufferLen, offset - DATA_DESCRIPTOR_LENGTH, function(err) {
		if (err) return emitErrorAndAutoClose(zipFile, err);
		if (
		...
		if (offset > zipFile.centralDirectoryOffset) return emitError(zipFile);
...
function found(zipFile, entry, cb) {
	...
	if (!zipFile.entryCountCertain) {
		...
		if (bytesRemaining > maxSize) {
			zipFile.entryCount += Math.ceil((bytesRemaining - maxSize) / CDH_MAX_LENGTH / SIXTY_FOUR_KIB) * SIXTY_FOUR_KIB;
			entriesRemaining = zipFile.entryCount - zipFile.entriesRead;
			zipFile.isFaulty = true;
		}

		// Check if entryCount is now certain
		if ((entriesRemaining + SIXTY_FOUR_KIB) * CDH_MIN_LENGTH > bytesRemaining) zipFile.entryCountCertain = true;
	}
